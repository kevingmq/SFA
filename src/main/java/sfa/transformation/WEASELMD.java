...
public class WEASELMD {
     ...
     * @return
     */
    public int[][][][] createWords(final MultiDimTimeSeries[] samples) {
     ...
     * @return
     */
    private int[][] createWords(final MultiDimTimeSeries[] samples, final int index, final int indexSource) {
    ...
    private TimeSeries[] splitMultiDimTimeSeries(int index, MultiDimTimeSeries[] samples) {
    ...
    public BagOfBigrams[] createBagOfPatterns(
        ...
        for (int j = 0; j < samples.length; j++) {
            bagOfPatterns[j] = new BagOfBigrams(words[0][0][j].length * 6 * 3, samples[j].getLabel());
